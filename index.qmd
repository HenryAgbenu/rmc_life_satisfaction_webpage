...
author: "Henry Agbenu" # Remember to add your name!
date: "19-10-2024"
theme: united
---
...
**Average self-reported life satisfaction** – Cantril Ladder score 
...
cantril_update <- cantril %>% 
  ...
  mutate(
  diff = `2023` - `2011`,  # Absolute change in scores
    percent_change = ((`2023` - `2011`) / `2011`) * 100  # Relative change from 2011 to 2023
  ) %>% 
  rename("Absolute change" = diff, "Relative change" = percent_change)

reactable(cantril_update)
...
cantril_line <- cantril %>% 
  filter(Country %in% c("Ghana", "South Africa","United States","France", "Israel", "Iran", "Jamaica")) %>% 
  ggplot(aes(x=Year, y= `Cantril ladder score`, group = Country, colour = Country))+
  geom_line()+
  scale_y_continuous(breaks = seq(0, 10, 1))+
  labs(
    title = "Line graph showing happiness score of selected countries overtime",x = "Year", y = "Cantril Ladder Score"
  )+
  theme_minimal()

ggplotly(cantril_line)
```
